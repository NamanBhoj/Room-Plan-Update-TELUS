...
namespace SilverTau.Sample
{
    public class Camera3DViewController : MonoBehaviour
    {
        ...
        private Bounds _targetObjectBounds;
        public Bounds GetTargetObjectBounds => _targetObjectBounds;
        ...
        private void OnEnable()
        {
            if (autoTarget)
            {
                ...
                {
                    var meshs = target.GetComponentsInChildren<MeshFilter>();
                    if (meshs.Length > 0)
                    {
                        _target = meshs[0].transform;
                    }
                }
            }
            else
            ...
            if(useObjectBounds) _targetObjectBounds = GetObjectBounds(target.gameObject);
            
            distanceBetweenCameraAndTarget = Vector3.Distance(sceneCamera.transform.position, useObjectBounds ? _targetObjectBounds.center : _target.position);
            dir = new Vector3(0, 0, distanceBetweenCameraAndTarget);//assign value to the distance between the maincamera and the target
            sceneCamera.transform.position = (useObjectBounds ? _targetObjectBounds.center : _target.position) + dir; //Initialize camera position
     
            cameraFOVDamp = sceneCamera.fieldOfView;
            cameraFieldOfView = sceneCamera.fieldOfView;
            sceneCamera.transform.LookAt(useObjectBounds ? _targetObjectBounds.center : _target.position);
        }
        
        private static Bounds GetObjectBounds(GameObject targetGameObject)
        {
            ...
            foreach (var meshFilter in targetGameObject.GetComponentsInChildren<MeshFilter>())
            {
                ...
                {
                    var multiplyPoint3X4 = localToWorld.MultiplyPoint3x4(vertex);
                    
                    if (!allPoints.Contains(multiplyPoint3X4))
                    {
                        allPoints.Add(multiplyPoint3X4);
                    }
                }
            }

            return GeometryUtility.CalculateBounds(allPoints.ToArray(), Matrix4x4.identity);
        }
        ...
        private void RotateCamera()
        {
            ...
            sceneCamera.transform.position = (useObjectBounds ? _targetObjectBounds.center : _target.position) + currentRot * dir;
            sceneCamera.transform.LookAt(useObjectBounds ? _targetObjectBounds.center : _target.position);
        }
